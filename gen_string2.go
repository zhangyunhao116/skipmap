...
func NewString2[valueT any]() *StringMap2[valueT] {
	var t valueT
	h := newStringNode2(0, "", t, maxLevel)
	h.flags.SetTrue(fullyLinked)
	return &StringMap2[valueT]{
		header:       h,
...
type StringMap2[valueT any] struct {
	length       int64
	highestLevel uint64 // highest level for now
	header       *stringnode2[valueT]
}

type stringnode2[valueT any] struct {
	hash  uint64
	key   string
...
func newStringNode2[valueT any](hash uint64, key string, value valueT, level int) *stringnode2[valueT] {
	node := &stringnode2[valueT]{
		hash:  hash,
		key:   key,
		level: uint32(level),
	}
...
func (n *stringnode2[valueT]) storeVal(value valueT) {
	atomic.StorePointer(&n.value, unsafe.Pointer(&value))
}

func (n *stringnode2[valueT]) loadVal() valueT {
	return *(*valueT)(atomic.LoadPointer(&n.value))
}

func (n *stringnode2[valueT]) loadNext(i int) *stringnode2[valueT] {
	return (*stringnode2[valueT])(n.next.load(i))
}

func (n *stringnode2[valueT]) storeNext(i int, node *stringnode2[valueT]) {
	n.next.store(i, unsafe.Pointer(node))
}

func (n *stringnode2[valueT]) atomicLoadNext(i int) *stringnode2[valueT] {
	return (*stringnode2[valueT])(n.next.atomicLoad(i))
}

func (n *stringnode2[valueT]) atomicStoreNext(i int, node *stringnode2[valueT]) {
...
func (s *StringMap2[valueT]) findNode(hash uint64, key string, preds *[maxLevel]*stringnode2[valueT], succs *[maxLevel]*stringnode2[valueT]) *stringnode2[valueT] {
	x := s.header
...
// The returned preds and succs always satisfy preds[i] > key >= succs[i].
func (s *StringMap2[valueT]) findNodeDelete(key string, preds *[maxLevel]*stringnode2[valueT], succs *[maxLevel]*stringnode2[valueT]) int {
...
func unlockstring2[valueT any](preds [maxLevel]*stringnode2[valueT], highestLevel int) {
	var prevPred *stringnode2[valueT]
...
func (s *StringMap2[valueT]) Store(key string, value valueT) {
	level := s.randomlevel()
	hash := strhash(key)
	var preds, succs [maxLevel]*stringnode2[valueT]
	for {
		...
		var (
			...
			valid                = true
			pred, succ, prevPred *stringnode2[valueT]
		)
		...
		if !valid {
			unlockstring2(preds, highestLocked)
			continue
		}

		nn := newStringNode2(hash, key, value, level)
...
func (s *StringMap2[valueT]) randomlevel() int {
...
func (s *StringMap2[valueT]) Load(key string) (value valueT, ok bool) {
	x := s.header
	hash := strhash(key)
	for i := int(atomic.LoadUint64(&s.highestLevel)) - 1; i >= 0; i-- {
...
func (s *StringMap2[valueT]) LoadAndDelete(key string) (value valueT, loaded bool) {
	var (
		nodeToDelete *stringnode2[valueT]
		isMarked     bool // represents if this operation mark the node
		topLayer     = -1
		preds, succs [maxLevel]*stringnode2[valueT]
	)
	for {
...
// (Modified from Store)
func (s *StringMap2[valueT]) LoadOrStore(key string, value valueT) (actual valueT, loaded bool) {
	var (
		level        int
		preds, succs [maxLevel]*stringnode2[valueT]
...
func (s *StringMap2[valueT]) LoadOrStoreLazy(key string, f func() valueT) (actual valueT, loaded bool) {
	var (
		level        int
		preds, succs [maxLevel]*stringnode2[valueT]
		hl           = int(atomic.LoadUint64(&s.highestLevel))
...
func (s *StringMap2[valueT]) Delete(key string) bool {
	var (
		nodeToDelete *stringnode2[valueT]
		isMarked     bool // represents if this operation mark the node
		topLayer     = -1
...
func (s *StringMap2[valueT]) Range(f func(key string, value valueT) bool) {
...
func (s *StringMap2[valueT]) Len() int {
